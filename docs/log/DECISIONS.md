...
### 2026-08-25 Day 6~7 스코프 축소 (시간 제약 6시간)

- 배경/문제: Day 6~7에 남은 총 작업 시간이 6시간으로 제한됨. PRD 6장 원문대로면 Day 6은
  코드 스플리팅 전/후 번들 트리맵, Lighthouse Performance/Accessibility 리포트, TTI
  계측값을 각각 "전/후" 비교로 확보하고 idle/listening/streaming/error/empty/권한거부 등
  7개 상태를 모두 갤러리로 구현해야 하고, Day 7은 데모 녹화 5종 + 컴포넌트 문서 + 성능·
  접근성 리포트 + README를 새로 작성해야 한다 — 이 전부를 6시간 안에 원문 그대로 완주하는
  건 비현실적.
...
- 결정: (B) 채택. 세부적으로: Day 6은 Lighthouse/번들 트리맵/TTI 계측을 각각 전/후 1회씩만
  빠르게 확보(반복 측정·재측정 없음), 상태 갤러리는 7개 상태 전부가 아니라 핵심 상태 위주로
  축소해서 구현. Day 7은 새 자료를 조사·작성하는 대신 기존 `DEVLOG.md`/`COMPONENT.md`/
  `DECISIONS.md`에 이미 쌓인 내용을 취합·정리하는 방식으로 진행.
- 이유: 사람이 명시적으로 요청한 시간 제약(6시간) 안에서 핵심 루프(알림→대화 시작→
  스트리밍 응답)의 완성도를 해치지 않으면서 현실적으로 끝낼 수 있는 유일한 방법. 기능 축소
  (대안 A)는 이미 Day 5까지 완성한 자동 턴테이킹/TTS 사이클을 되돌리는 것이라 손실이 더 크고,
  원문 그대로 진행(대안 C)은 시간 내 완주가 불가능해 미완료 산출물만 남긴다.
- 영향받는 범위: 앞으로 Day 6/7 관련 모든 작업 판단 기준. 이 결정은 `docs/rules/PRD.md` 6장/
  7장 원문보다 우선 적용한다 — PRD 원문은 참고용으로 남겨두되, 실제 작업 범위는 이 항목
  기준으로 판단한다.

### 2026-08-25 Day 6~7 남은 시간 6시간 → 4시간 재축소, 데모 녹화 5종 → 3종

- 배경/문제: 위 항목("Day 6~7 스코프 축소")에서 6시간 기준으로 재계획한 뒤, 남은 시간이
  4시간으로 다시 줄어듦. 기존 축소안(계측 1회씩, 상태 갤러리 핵심만, Day 7 취합 위주)
  그대로도 4시간 안에 들어갈 수 있는지 재점검 필요.
- 검토한 대안(현실성 재점검 결과): 항목별 예상 소요시간을 다시 추정함 — Day 6(코드
  스플리팅 15분 + 번들 트리맵 전/후 15분 + TTI 계측 전/후 20분 + Lighthouse 전/후 25분 +
  접근성 빠른 점검 15분 + 상태 갤러리 핵심 상태 30분 ≈ 2시간) + Day 7(데모 녹화 5종
  50~75분 + 컴포넌트 문서 링크 10분 + 성능·접근성 리포트 취합 15분 + README 20분 ≈
  1.5~2시간) = 합계 3.5~4시간. 여유가 0에 가까워 Lighthouse 재측정이나 코드 스플리칭
  중 버그 하나만 생겨도 바로 초과되는 구조 — 가장 변동성이 큰 항목은 데모 녹화 5종.
  (A) 데모 녹화를 3종으로 축소해 여유 확보, (B) 5종 유지하고 초과 리스크 감수, (C) 녹화는
  유지하고 Day 6 계측 쪽을 더 압축, 세 가지를 사람에게 제시해 확인받음.
- 결정: (A) 채택. 데모 녹화는 ① 자동 턴테이킹 3~5턴 전체 사이클, ② 크롬 개발자도구
  Network 탭 스트리밍 청크 증빙, ③ 마이크 미지원/거부 시 텍스트 폴백, 3종만 녹화한다.
  오탐 발생 후 "이어서 말하기" 복구 장면, 스크린리더 시연 장면은 이번 녹화 스코프에서
  제외한다(기능 자체는 유지 — 코드/동작은 그대로 두고 별도 영상 증빙만 생략).
- 이유: 사람이 직접 선택함(권장안 채택). 5종 녹화가 4시간 예산에서 가장 변동성 큰 항목이라
  여기를 줄이는 게 다른 계측 작업(Lighthouse/TTI/번들)의 정확성을 희생하지 않고 여유를
  만드는 가장 리스크 낮은 방법.
- 영향받는 범위: Day 7 데모 자료 수집 항목, `docs/deliverables/CHECKLIST.md`의 "데모 녹화
  5종" 항목(발표 시 3종만 첨부하고 사유를 이 문서로 링크). 오탐 복구/스크린리더 관련
